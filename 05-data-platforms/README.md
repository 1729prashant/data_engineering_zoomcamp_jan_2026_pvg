...
```
Ans -> `.bruin.yml` and `pipeline/` with `pipeline.yml` and `assets/`
REASON: see the readme generated by bruin init zoomcamp my-pipeline, otherwise you only find out when one of these is missing and you try to run the pipeline. as of my answer the bruin documentation does not spell this out explicitly.
```
...
```
Ans -> bruin run --var 'taxi_types=["yellow"]'
REASON: 
```
...
```
Ans -> bruin run ingestion/trips.py --downstream
REASON: 
```
...
```
Ans -> bruin lineage
REASON: https://getbruin.com/docs/bruin/commands/lineage.html
```
...
```
Ans -> --full-refresh
REASON: https://getbruin.com/docs/bruin/commands/run.html#examples
```
