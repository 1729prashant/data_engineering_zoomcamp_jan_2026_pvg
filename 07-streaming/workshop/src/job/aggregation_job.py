...
# This script defines a PyFlink job that performs time-windowed aggregation
# on taxi ride events from a Kafka topic and writes the aggregated results
...
# where aggregated events will be stored. This table includes a primary key to
# support upsert operations, which is crucial for handling late-arriving data
# in streaming aggregations.
...
# from which taxi ride events will be read. Crucially, it defines an event_timestamp
# and a WATERMARK, which are essential for correct time-windowed processing in Flink.
            # Define a computed column 'event_timestamp' from 'tpep_pickup_datetime'.
            # TO_TIMESTAMP_LTZ converts epoch milliseconds to a timestamp with local time zone.
            ...
            # 'properties.auto.offset.reset' = 'earliest', # Kafka consumer property for offset reset.
...
# --- Main Flink Job Logic for Aggregation ---
# This function orchestrates the Flink streaming job for aggregation.
def log_aggregation():
    # Set up the Stream Execution Environment.
    env = StreamExecutionEnvironment.get_execution_environment()
    # Enable checkpointing every 10 seconds for fault tolerance.
    env.enable_checkpointing(10 * 1000)
    # Set the default parallelism for the job. This determines how many parallel instances
    # of operators Flink will try to run.
    ...
    try:
        # Create and register the Kafka source table with watermark definition.
        ...
        # This query uses a TUMBLE window function to group events into 1-hour non-overlapping windows.
        # Events are grouped by 'window_start' (the beginning of the hour) and 'PULocationID'.
        # It counts the number of trips and sums the total revenue for each group.
        t_env.execute_sql(f"""
